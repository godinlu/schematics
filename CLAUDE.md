...
Required `.env` file at `htdocs/schematics/.env` (not committed):
```
DB_HOST=
DB_NAME=
DB_USER=
...
- **`generateSchema.php`** — Main schema generation endpoint. Accepts `?image=<type>&format=<fmt>&debug=1`. Acts as factory routing to the appropriate generator class.
...
Vanilla JS (ES6 modules), no framework. Each page declares `$css` and `$js` arrays, loaded by `includes/head.php`. Form state is persisted via `sessionStorage`. Pages call `api/generateSchema.php` asynchronously to render schema previews.
...
- Python test scripts for the API are in `dev/` (`testApi.py`, `testHTTP.py`).
